...
If you want to estimate the language of a text, reference texts in known languages are needed. In `koRpus`, the [Universal Declaration of Human Rights with its more than 350 translations](http://www.unicode.org/udhr/download.html) is used.
...
```
<!-- cache must be off, otherwise the correction further down might not work reliably -->
```{r, echo=FALSE}
(hyph.txt.en <- hyphen(tagged.text, quiet=TRUE, cache=FALSE))
```
