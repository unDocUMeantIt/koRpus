...
For this class of objects, \texttt{koRpus} provides some comfortable methods to extract the portions you're interested in. For example, the main results are to be found in the slot \texttt{TT.res}. In addition to TreeTagger's original output (token, tag and lemma) \texttt{treetag()} also automatically counts letters and assigns tokens to global word classes. To get these results as a data.frame, use the getter method \texttt{taggedText()}:
...
\subsubsection{Accessing data from \texttt{koRpus} objects}

In case you want to access a subset of the data in the resulting object, e.\,g., only the column with number of letters or the first five rows, you'll be happy to know there's special \texttt{[} and \texttt{[[} methods for these kinds of objects:
...
\begin{description}
  ...
  \item[\texttt{letters.only}:] Counts only letters, excluding(!) digits (which are counted seperately as \texttt{digits})
\end{description}
...
To analyze the lexical diversity of our text we can now simply hand over the tagged text object to the \texttt{lex.div()} method:
...
Since this package is intended for research, it is possible to directly influence all relevant values of each measure and examine the effects. For example, as mentioned before \texttt{segment.optimizer()} recommended a change of segment size for MSTTR to drop less words, which is easily done:
...
The package comes with implementations of several readability formulae. Some of them depend on the number of syllables in the text.\footnote{Whether this is the case can be looked up in the documentation.} To achieve this, the method \texttt{hyphen()} takes objects of class \texttt{kRp.tagged} and applies an hyphenation algorithm \cite{liang_word_1983} to each word. This algorithm was originally developed for automatic word hyphenation in \LaTeX{}, and is gracefully misused here to fulfill a slightly different service.\footnote{The \texttt{hyphen()} method was originally implemented as part of the \texttt{koRpus} package, but was later split off into its own package called \texttt{sylly}.}
...
So, the more accurate the end results should be, the less you should rely on the automatic hyphenation alone. But it sure is a good starting point, for there is a function called \texttt{correct.hyph()} to help you clean these results of errors later on. The most comfortable way to do this is to call \texttt{hyphenText(hyph.txt.en)}, which will get you a data frame with two colums, \texttt{word} (the hyphenated words) and \texttt{syll} (the number of syllables), in a spread sheet editor:\footnote{For example, this can be comfortably done with RKWard: \url{https://rkward.kde.org}}
...
The \texttt{summary()} method supports an additional flat format, which basically turns the table into a named numeric vector,
...
The language support of this package has a modular design. There are some pre-built language packages in the \texttt{l10n} repository,\footnote{\url{https://undocumeantit.github.io/repos/}} and with a little effort you should be able to add new languages yourself. You need the package sources for this, then basically you will have to add a new file to it and rebuild/reinstall the package. More details on this topic can be found in \texttt{inst/README.languages}. Once you got a new language to work with \texttt{koRpus}, I'd be happy to include your module in the official distribution.

\section{Analyzing full corpora}
Despite its name, the scope of \texttt{koRpus} is single texts. If you would like to do analysis an a full corpus of texts, have a look at the plugin package \texttt{tm.plugin.koRpus}.\footnote{\url{https://reaktanz.de/R/pckg/tm.plugin.koRpus/} (package repo) or \url{https://github.com/unDocUMeantIt/tm.plugin.koRpus} (source code)}
